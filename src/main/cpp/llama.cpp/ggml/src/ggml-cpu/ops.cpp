...
static void ggml_compute_forward_rms_norm_f32(
    ...
    // Single-pass fused RMSNorm: read x once (sum-of-squares accumulated as we
    // go), then write y = x * scale in one second pass. Drops to 1 read + 1
    // write per row vs. the previous 1 read + memcpy + scale-in-place (which
    // touched the row 3×). On a memory-bandwidth-bound CPU decode this is a
    // ~30% reduction in the bytes this op transfers — small absolute win per
    // call but the op runs on every layer, twice (attn_norm + ffn_norm) per
    // model arch like llama / qwen / lfm.
    //
    // Numerically equivalent: sum-of-squares is computed in ggml_float, mean
    // = sum/ne00, scale = 1/sqrt(mean + eps), then y[i] = x[i] * scale. Same
    // arithmetic as before, just one fewer trip through DRAM.
    for (int64_t i03 = 0; i03 < ne03; i03++) {
        for (int64_t i02 = 0; i02 < ne02; i02++) {
            for (int64_t i01 = ith; i01 < ne01; i01 += nth) {
                const float * x = (float *) ((char *) src0->data + i01*nb01 + i02*nb02 + i03*nb03);
                ...
                if (y == x) {
                    ...
                    // out-of-place: fused multiply-write
                    for (int64_t i00 = 0; i00 < ne00; i00++) {
                        y[i00] = x[i00] * scale;
                    }
                }
